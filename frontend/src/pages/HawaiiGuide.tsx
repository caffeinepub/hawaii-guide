...
import { MapPin, Wind, Sun, Waves, ChevronDown, Star, Clock, Calendar, Thermometer, Heart, UtensilsCrossed } from 'lucide-react';
...
export default function HawaiiGuide() {
  ...
  return (
    <div className="min-h-screen bg-sand font-body">
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <button
              ...
              className="bg-white/15 hover:bg-white/25 backdrop-blur-sm border border-white/30 text-white font-semibold px-8 py-4 rounded-full text-lg transition-all duration-300"
            >
              Discover Dining
            </button>
          </div>
      ...
      <section className="relative py-20 overflow-hidden bg-ocean-deep">
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center px-4">
          ...
          <p className="text-white/70 text-lg mb-8 leading-relaxed">
            The islands are calling. Pack light, bring sunscreen, and leave your worries on the mainland.
          </p>
          <button
            onClick={() => scrollTo(destinationsRef)}
            className="bg-golden hover:bg-golden-dark text-ocean-deep font-bold px-10 py-4 rounded-full text-lg transition-all duration-300 hover:shadow-xl hover:shadow-golden/30 hover:-translate-y-0.5"
          >
      ...
      {/* Footer */}
      <footer className="bg-ocean-deep border-t border-white/10 py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-3">
              <span className="text-3xl">🌺</span>
              <div>
                <div className="font-display text-xl font-bold text-white">Aloha Hawaii</div>
                <div className="text-white/50 text-sm">Your Ultimate Island Guide</div>
              </div>
            </div>

            <div className="flex flex-wrap justify-center gap-6 text-sm text-white/50">
              {['Oahu', 'Maui', 'Big Island', 'Kauai'].map((island) => (
                <button
                  key={island}
                  onClick={() => scrollTo(destinationsRef)}
            ...
            <div className="text-center md:text-right">
              <p className="text-white/40 text-sm">
                © {new Date().getFullYear()} Aloha Hawaii Guide
              </p>
              <p className="text-white/30 text-xs mt-1 flex items-center justify-center md:justify-end gap-1">
                Built with{' '}
                <Heart className="w-3 h-3 text-coral fill-coral inline" />{' '}
                using{' '}
                <a
                  href={`https://caffeine.ai/?utm_source=Caffeine-footer&utm_medium=referral&utm_content=${encodeURIComponent(typeof window !== 'undefined' ? window.location.hostname : 'hawaii-guide')}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-golden/60 hover:text-golden transition-colors"
                >
                  caffeine.ai
                </a>
              </p>
